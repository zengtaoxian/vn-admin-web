...
<template>
  <div class="box">
    <div class="main_header clear">
      <div class="input_wrap" v-if="havePerm('197')">
    ...
    <div class="new_tab_wrap" ref="new_tab_wrap" :style="{height:tabHeight+'px'}">
      <table class="new_table">
        ...
        <tr class="tba_content" v-for="(item,index) in listData">
          ...
          <td class="pad_lf">
            <span class="tab_del_btn" @click="showDetail(item.id,item.packageType)" v-if="havePerm('283')">详情</span>

            <span v-if="havePerm('273') && item.status == '下架'">
              <Poptip
                transfer
                placement="top-end"
                confirm
                title="您确定要上架这条价格包吗？"
                @on-ok="clickUpdatePriceOn(item)"
                @on-cancel="cancel">
                  <span class="tab_del_btn">上架</span>
                </Poptip>
            </span>
            <span v-if="havePerm('274') && item.status == '上架'">
              <Poptip
                transfer
                placement="top-end"
                confirm
                title="您确定要下架这条价格包吗？"
                @on-ok="clickUpdatePriceOff(item)"
                @on-cancel="cancel">
                  <span class="tab_del_btn">下架</span>
                </Poptip>
            </span>
            <span class="tab_del_btn" @click="editPricel(item.id)" v-if="item.onTime == '' && havePerm('276')">编辑</span>
            <span v-if="item.onTime == ''">
              <Poptip
                transfer
                placement="top-end"
                confirm
                title="您确定要删除这个价格包吗？"
                @on-ok="delPricel(item.id)"
                @on-cancel="cancel">
                  <span class="tab_del_btn">删除</span>
                </Poptip>
            </span>
            <!--<span class="tab_del_btn" @click="clickUpdatePriceOn(item)" v-if="havePerm('273') && item.status == '下架'">上架</span>-->
            <!--<span class="tab_del_btn" @click="clickUpdatePriceOff(item)" v-if="havePerm('274') && item.status == '上架'">下架</span>-->
          </td>
        </tr>
      </table>
    </div>
    ...
    <Modal class="vertical-center-modal" :mask-closable="false" v-model="modal1" title="新增挂牌价格包"
           @on-ok="handleSubmit('formValidate')"
           :scrollable="scrollable"
           @on-cancel="handleReset('formValidate')" width="920">
            ...
            <TabPane label="基本资料" name="tab1">
              <div class="tab_content">
                ...
                <div class="tab_line clear">
                  ...
                  <div class="tab_line_right" v-html="priceMap.chargeTypeName"></div>
                </div>
                <div class="tab_line clear">
                  <div class="tab_line_left">线路类型</div>
                  <div class="tab_line_right" v-html="priceMap.eTypeName"></div>
                </div>
                <div class="tab_line clear">
                ...
                <div class="tab_line clear">
                  ...
                  <div class="tab_line_right" v-html="priceMap.remark" title="aaaaaaaaaaaaaaaaaaaaaa"></div>
                </div>
                <div class="tab_line clear">
    ...
    <!-- 编辑开始 -->
    <Modal class="vertical-center-modal" :mask-closable="false" v-model="modalEdit" title="编辑挂牌价格包"
           @on-ok="updatePrice('formValidate')"
           :scrollable="scrollable"
...
//        this.$Message.info("点击了取消");
      ...
      showPricelAdd(val) {
        this.formValidate = JSON.parse(JSON.stringify(this.initFormValidate));
        this.formValidate.chargeType = "1";
          ...
          if (res.data.respCode == '0') {
            ...
            api.fetchPriceDetail({
              ...
              if (res.data.respCode == '0') {
                ...
                this.formValidate.goodType = parseInt(this.formValidate.goodType);
