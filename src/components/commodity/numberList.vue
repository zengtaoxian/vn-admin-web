...
<template>
  <div class="box">
    ...
    <div class="main_header clear">
      <div class="input_wrap" v-if="havePerm('293')">
        <i-input placeholder="号码商品编号/号码/资源类型/归属城市/关联订单" style: @on-enter="search" v-model.trim="condition"></i-input>
      ...
      <!--</div>-->
      <!--{{cityDataVal}}-->
      <div class="header_btn" v-if="havePerm('297')">
      ...
      <div class="export_btn" @click="exportData" v-if="havePerm('294')"></div>
    </div>
      ...
      <div>
        <table class="new_table">
          ...
          <tr class="tab_th">
            ...
            <th class="text_left pad_lf">号码</th>
            <th class="text_left pad_lf">成本包</th>
            <th class="text_left pad_lf">挂牌包</th>
            <th class="text_left pad_lf">号码商品编号</th>
          ...
          <tr class="tba_content" v-for="(item,index) in listData">
            ...
            <td class="pad_lf" v-html="item.number"></td>
            <td class="pad_lf" v-html="item.cb_packageName">成本包</td>
            <td class="pad_lf" v-html="item.gp_packageName">挂牌包</td>
            <td class="pad_lf" v-html="item.numberNo">号码商品编号</td>
            <td class="pad_lf" v-html="item.supplierName">供应商</td>
            <td class="pad_lf" v-html="item.cityName">归属城市</td>
            <td class="pad_lf" v-html="item.supplyTypeName">归属运营商</td>
            <td class="pad_lf" v-html="item.typeName">资源类型</td>
            <td class="pad_lf" v-html="item.createTime">创建时间</td>
            <td class="pad_lf" v-html="item.createUser">创建人</td>
            <td class="pad_lf" v-html="item.onTime">最后上架时间</td>
            <td class="pad_lf" v-html="item.offTime">最后下架时间</td>
            <td class="pad_lf" v-html="item.status">状态</td>
            <td class="pad_lf" v-html="item.orderId">关联订单</td>
            <td class="pad_lf">
              <span class="tab_del_btn" @click="clickShowModal(item)"
                    v-if="havePerm('291') && item.status == '下架'">上架</span>
              <span class="tab_del_btn" @click="clickShowModal(item)"
                    v-if="havePerm('292') && item.status == '上架'">下架</span>
              <span class="tab_del_btn" @click="showDetail(item.id)" v-if="havePerm('298')">查看详情</span>
              <span class="tab_del_btn"
                    ...
                    v-if="havePerm('295')">调整挂牌价</span>
              <span class="tab_del_btn"
                    @click="showCbPrice(item.id,item.chengbenId,item.chargeType,item.eType,item.cb_packageName,item.supplierName)"
                    v-if="havePerm('296')">调整成本价</span>
              <span style="position: relative;" v-if="item.onTime == ''&& havePerm('299')">
                <Poptip
                  transfer
                  placement="top-end"
                  confirm
                  title="您确认删除这条内容吗？"
                  @on-ok="deleteNumbGoods(item.id)"
                  @on-cancel="cancel">
    ...
    <Modal class="vertical-center-modal" v-model="modal2" :title="addGoodTitle" @on-ok="handleSubmit()"
      ...
      <Steps :current="current" style="width: 60%;margin: 0 auto;padding:20px 40px;">
        <Step title="号码基本信息"></Step>
        <Step title="号码商品配置"></Step>
      </Steps>
      <div v-show="current == 0">
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">
          <Row>
            <Col span="22" offset="1" class="role">
            <FormItem class="ivu-form-item-required" label="号码名称">
              <Input v-model="formValidate.number" placeholder="2~50个字"></Input>
            </FormItem>

            <FormItem class="ivu-form-item-required" label="号码类别">
        ...
        <Form ref="formValidate" :label-width="100">
          ...
          <Row>
            ...
            <div class="footer_modal">
              ...
              <Button type="primary" class="sub_btn_size" @click="handleSubmit('formValidate')"
                      v-html="(formValidate.id && formValidate.id != '')?'保存号码商品':'生成号码商品'"></Button>
            </div>
            </Col>
          </Row>
        </Form>
      ...
      <div>
        <div slot="header">
          <div class="detail_top clear">
            ...
            <div class="detail_top_right">
              <p class="title" v-html="goodDetail.number"></p>
              <p class="code" v-html="goodDetail.status"></p>
            </div>
            <span @click="editShowGood(goodDetail.id)" class="top_btn"
                  v-if="goodDetail.onTime == '' && havePerm('300')">
        ...
        <div>
          <Tabs :value="GoodTab" v-model="GoodTab" class="tabs_check" type="card" :animated="animated"
                ...
                <ul class="tabs_detail_con_ul">
                  <li class="tabs_detail_con_li">
                    ...
                    <div class="tabs_detail_right">
                      <p class="tabs_msg" v-html="goodDetail.chargeTypeName"></p>
                  ...
                  <li class="tabs_detail_con_li">
                    ...
                    <div class="tabs_detail_right">
                      <p class="tabs_msg" v-html="goodDetail.supplyTypeName"></p>
                  ...
                  <li class="tabs_detail_con_li">
                    ...
                    <div class="tabs_detail_right">
                      <p class="tabs_msg" v-html="goodDetail.remark?goodDetail.remark:'-'" :title="goodDetail.remark"></p>
                      <p class="tdetail_msg_text">号码商品说明</p>
                    </div>
                  </li>
                  <li class="tabs_detail_con_li">
            ...
            <TabPane label="号码商品配置" name="GoodTab2">
    ...
    <!-- 调整挂牌价 开始 -->
    <Modal v-model="isGpPrice" :title="priceTitle" :mask-closable="false" @on-ok="hideGpPrice" @on-cancel="hideGpPrice"
           width="640">
      <Form ref="formValidate" :label-width="100">
        <Row>
    ...
    <!-- 调整成本价 -->
    <Modal v-model="isCbPrice" :title="priceTitle" :mask-closable="false" @on-ok="hideCbPrice()"
           @on-cancel="hideCbPrice" width="640">
      <Form ref="formValidate" :label-width="100">
        <Row>
          <Col span="20" offset="1" class="role">
          <FormItem class="" style="margin-bottom: 0px;" label="供应商">{{supplierName}}</FormItem>
            ...
            <TabPane label="基本资料" name="priceTab1">
              <div class="tab_content">
                ...
                <div class="tab_line clear">
                  ...
                  <div class="tab_line_right" v-html="priceMap.remark" :title="priceMap.remark"></div>
                </div>
                <div class="tab_line clear">
        ...
        <div style="padding:10px 0 20px;">
          ...
          <div class="upload_div" v-if="havePerm('40')">
            <Upload :action="actionUrl" :on-success="uploadFileSuccess" :on-error="uploadFileError" accept=".xlsx,.xls">
              <Button type="primary" icon="ios-cloud-upload-outline">导入商品号码</Button>
            </Upload>
          </div>
        </div>
      </div>
  ...
  export default {
    data() {
      return {
        ...
        ucploadModal2: false,
        ucploadMsg: '',
        actionUrl: '/vcloud/goodManage/number/uploadNumberExcel',
        disabled: true,
        animated: false,
        clearable: true,
        tabHeight: 500,
    ...
    created() {
      ...
      this.actionUrl = '/vcloud/goodManage/number/uploadNumberExcel?Authorization=' + this.Authorization + '&createUser=' + this.createUser;
//      this.getcity();
    },
    methods: {
      gatewayChange(item) {
...
//        this.$Message.info("点击了取消");
      ...
      exportData(type) {
        ...
        api.DownLoadFile({
          url: '/vcloud/goodManage/number/exportNumberList', data: {
      ...
      clickUpdateOff(item) {
        ...
        this.formValidate.createUser = localStorage.getItem('USER');
        api.fetchNumberUpdateStatusOff({
          id: item.id,
          email: this.remindEmail,
          createUser: this.formValidate.createUser
        }).then((res) => {
      ...
      updateOn() {
        ...
        if (this.canOn == "1") {
          this.canOn = "0";
          this.$Message.info("号码商品存在待配置项,不可上架");
          return
        }
      ...
      editShowGood(id) {
        ...
        this.formValidate.number = this.goodDetail.number;
        this.formValidate.goodType = parseInt(this.goodDetail.goodType);
        this.formValidate.chargeType = this.goodDetail.chargeType;
        this.formValidate.supplyType = this.goodDetail.supplyType;
...
//            eType: eType
          ...
          if (res.data.respCode == '0') {
            ...
            console.log(this.packageTypeGp);
          }
        }).catch((err) => {
          console.log(err)
        })
...
//            eType: eType
      ...
      handleSubmit: function (name) {
        ...
        if (!flag) {
          ...
          if (this.formValidate.id == '') {
            //  新增
            api.fetchNumbGoodAdd(
              this.formValidate
            ).then((res) => {

              if (res.data.respCode == '0') {
                this.$Message.info("号码商品添加成功");
                this.modal2 = false;
                this.formValidate = JSON.parse(JSON.stringify(this.initFormValidate));
                this.getNumberList();
              } else {
            ...
            // 更新
            api.fetchUpdataNumbGood(this.formValidate).then((res) => {
              if (res.data.respCode == '0') {
                this.$Message.info("编辑号码商品成功");
                this.modal2 = false;
                this.formValidate = JSON.parse(JSON.stringify(this.initFormValidate));
                this.getNumberList();
              } else {
      ...
      addGoodValidateData1() {
        ...
        if (this.formValidate.number == "") {
          this.$Message.info("号码商品名称不为空");
          return false;
        }

        if (this.formValidate.number.length < 2 || this.formValidate.number.length > 50) {
          this.$Message.info("号码商品名称长度在2-50之间");
          return false;
        }

        if (this.formValidate.goodType == "") {
          this.$Message.info("号码商品类别不为空");
          return false;
        }
      ...
      goToexport() {
        ...
        api.DownLoadFile({
          url: '/vcloud/goodManage/number/exportErrExcel_num', data: {
...
//        下载模板
        api.DownLoadFile({
          url: '/vcloud/goodManage/number/exportDefExcel', data: {}
        });
      },
      getcity() {
