...
<template>
  <div class="box">
    <div class="main_header clear">
      <div class="input_wrap" v-if="havePerm('232')">
    ...
    <div class="new_tab_wrap" ref="new_tab_wrap" :style="{height:tabHeight+'px'}">
      <table class="new_table">
        ...
        <tr class="tba_content" v-for="(item,index) in listData">
          ...
          <td class="pad_lf">
            <span class="tab_del_btn" @click="showDetail(item.id,item.packageType)" v-if="havePerm('284')">详情</span>

            <span v-if="havePerm('278') && item.status == '下架'">
              <Poptip
                transfer
                placement="top-end"
                confirm
            ...
            <span v-if="havePerm('279') && item.status == '上架'">
              <Poptip
                transfer
                placement="top-end"
                confirm
            ...
            <span class="tab_del_btn" @click="editPricel(item.id)" v-if="item.onTime == '' && havePerm('281')">编辑</span>
    ...
    <div class="page_btm_div" v-if="totalCount">
      <div class="left_btn" v-show="checkBoxArr.length>0">
        ...
        <span class="span_btn" @click="updatePriceOn()" v-if="havePerm('278')">上架</span>
        <span class="span_btn" @click="updatePriceOff()" v-if="havePerm('279')">下架</span>
      </div>
    ...
    <Modal class="vertical-center-modal" :mask-closable="false" v-model="modal1" title="新增成本价格包" @on-ok="handleSubmit('formValidate')"
    ...
    <!-- 编辑开始 -->
    <Modal class="vertical-center-modal" :mask-closable="false" v-model="modalEdit" title="编辑成本价格包" @on-ok="updatePrice('formValidate')"
           :scrollable="scrollable"
           @on-cancel="handleResetEdit" width="920">
      <Row>
